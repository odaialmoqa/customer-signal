...
export async function GET(request: NextRequest) {
  try {
    ...
    const storyClusters = await trendAnalysisService.getStoryClusters(profile.tenant_id, options)

    return NextResponse.json({ storyClusters })
  } catch (error) {
    console.error('Error getting story clusters:', error)
    return NextResponse.json(
      { error: 'Failed to get story clusters' },
      { status: 500 }
    )
  }
}